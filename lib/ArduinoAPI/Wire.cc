...
uint8_t TwoWire::endTransmission(uint8_t sendStop)
{
  ...
  LL_I2C_TransmitData8(I2C1, (txAddress << 1) | LL_I2C_DIRECTION_WRITE);
  while (!LL_I2C_IsActiveFlag_ADDR(I2C1));
...
u32 TwoWire::write(uint8_t data)
{
  if(transmitting){
...
// slave tx event callback
// or after beginTransmission(address)
u32 TwoWire::write(const uint8_t *data, u32 quantity)
{
  if(transmitting){
  // in master transmitter mode
